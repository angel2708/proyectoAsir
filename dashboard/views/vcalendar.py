...
from dashboard.models import AbsenceMedia, Companies, Account, Departament, Calendar, CA, Absence
...
def saveEvent (request):
    ...
    if request.user.is_authenticated:
        ...
        if request.POST.get('action') == 'save_event':
...
def saveHolidays (request):
    #Comprobamos que el usuario está logueado
    if request.user.is_authenticated:

        # Detectar la actualización de usuario.
        if request.POST.get('action') == 'save_holidays':
            user_id = request.POST.get('user_id')
            company_id = request.POST.get('company_id')
            departament_id = request.POST.get('departament_id')
            title = request.POST.get('title')
            ...
            if str(manager) == str(user_id):
                state = 'Aceptadas'
            else:
                state = 'Pendientes'

            vacation_days = request.POST.get('vacation_days')
            ...
            vacations = Calendar.objects.raw("SELECT * FROM dashboard_calendar WHERE id="+str(holidays.id))
            tmpJson = serializers.serialize("json",vacations)
            tmpObj = json.loads(tmpJson)
            json_vacations = json.dumps(tmpObj)

            account = Account.objects.raw("SELECT * FROM dashboard_account WHERE id="+str(holidays.user_id))
            tmpJson = serializers.serialize("json",account)
            tmpObj = json.loads(tmpJson)
            json_account = json.dumps(tmpObj)

            departament = Departament.objects.raw("SELECT * FROM dashboard_departament WHERE id="+str(holidays.departament_id))
            tmpJson = serializers.serialize("json",departament)
            tmpObj = json.loads(tmpJson)
            json_departament = json.dumps(tmpObj)

            return HttpResponse('{"vacations":'+json_vacations+',"departament":'+json_departament+',"account":'+json_account+'}')
...
def vacations(request):
    ...
    if request.user.id == 18 or request.user.id == 64: # Estos usuarios van a ver las ausencias de TODOS los empleados de la empresa
        ...
        absences = Absence.objects.raw("SELECT dashboard_absence.*, dashboard_account.first_name, dashboard_account.last_name, dashboard_absencemedia.file FROM dashboard_absence JOIN dashboard_account ON dashboard_account.id = dashboard_absence.user FULL OUTER JOIN dashboard_absencemedia ON dashboard_absencemedia.absence_id = dashboard_absence.id WHERE dashboard_absence.deleted = 0 AND dashboard_account.is_active = true AND departament = "+str(departament_id))
        for i in range(len(absences)):
            absences[i].description='<br>'.join(l for line in absences[i].description.splitlines() for l in textwrap.wrap(line, width=50))
    ...
    if request.POST.get('action') == 'add_absence':
        ...
        abs_purchases = Absence.objects.raw(
            "SELECT dashboard_absence.* FROM dashboard_absence WHERE dashboard_absence.departament = 3 AND dashboard_absence.deleted = 0 AND dashboard_absence.start = '" + start + "' AND dashboard_absence.finish = '" + finish + "'")
        abs_sales = Absence.objects.raw(
            "SELECT dashboard_absence.* FROM dashboard_absence WHERE dashboard_absence.departament = 4 AND dashboard_absence.user = " + str(this_user_ca.mirror) + " AND dashboard_absence.deleted = 0 AND dashboard_absence.start = '" + start + "' AND dashboard_absence.finish = '" + finish + "'")
        
        # Vacaciones compras solo un usuario por semana
        if int(departament) == 3 and int(len(abs_purchases)) > 0 and int(diff) == 7:
            ...
            return HttpResponse('compras/*/*'+str(name_user))
        ...
        if absence_type == str(1):
            ...
            try:
                ...
                #Descomentar Email
                if str(departament) == '4':
                    mail = create_mail('miguel.mancha@esmovil.es', subject, template_name, {'host':host, 'username':username, 'absence_type':absence_type2, 'description':description, 'start':start2, 'finish':finish2})
                    ...
                    mail.send(fail_silently=False)
                mail = create_mail('jose.segura@esmovil.es', subject, template_name, {'host':host, 'username':username, 'absence_type':absence_type2, 'description':description, 'start':start2, 'finish':finish2})
                mail.send(fail_silently=False)
            except:
    ...
    if request.POST.get('action') == 'accept_absence':
        ...
        if str(abs.absence_type) == str(6):
            if (int(pending_days)-int(diff)) >= 0:
                abs = Absence.objects.get(pk=ida)
                abs.state = 1
                abs.save()
    ...
    if request.POST.get('action') == 'cancel_absence':
        ...
        if str(manager_id) == str(request.user.id) or str(request.user.id) == '45' or str(request.user.id) == '51':
            ...
            try:
                host = request.META['HTTP_HOST']
                if str(request.user.id) == '45':
                    subject = "Miguel Mancha ha rechazado tu solicitud de ausencia"
                elif str(request.user.id) == '51':
                    subject = "Ernesto Duarte ha rechazado tu solicitud de ausencia"
                else:
                ...
                mail.send(fail_silently=False)
                mail = create_mail('jose.segura@esmovil.es', subject, template_name, {'host':host, 'username':username, 'manager_username':manager_username, 'absence_type':absence_type2, 'description':abs.description, 'start':start2, 'finish':finish2})
                mail.send(fail_silently=False)
            except:
                print("Límite de correos alcanzado")
    ...
    if request.user.id == 18 or request.user.id == 64: #Usuarios Josemi y Juanfran - Deben ver todos los usuario de la empresa
        users = Account.objects.raw("SELECT dashboard_ca.*, dashboard_account.id, dashboard_account.first_name, dashboard_account.last_name, dashboard_account.avatar FROM dashboard_account INNER JOIN dashboard_ca ON dashboard_account.id = dashboard_ca.user_id WHERE dashboard_account.is_active = true AND dashboard_ca.company_id="+str(company_id)+" ORDER BY dashboard_account.first_name")
    else:
        users = Account.objects.raw("SELECT dashboard_ca.*, dashboard_account.id, dashboard_account.first_name, dashboard_account.last_name, dashboard_account.avatar FROM dashboard_account INNER JOIN dashboard_ca ON dashboard_account.id = dashboard_ca.user_id WHERE dashboard_account.is_active = true AND dashboard_ca.departament_id="+str(departament_id)+" AND dashboard_ca.company_id="+str(company_id)+" ORDER BY dashboard_account.first_name")
    ...
    try:
        ...
        return render(request, 'vacations/vacations.html',{'vacations':vacations, 'ca':ca, 'month':month, 'dept_name':dept_name, 'users':users, 'departaments':departaments, 'role':ca.role, 'absences':absences, 'total_vacation_days':total_vacation_days, 'vacation_days':vacation_days, 'vacation_days_next':vacation_days_next, 'pending_days':pending_days, 'free_days':free_days, 'total_free_days':total_free_days})
    except:
        return render(request, 'accessDenied/accessDenied.html')

# ACEPTAR PETICIONES DE VACACIONES QUE AUN NO HAN SIDO ACEPTADAS
def acceptPendingVacations(request):
    #Comprobamos que el usuario está logueado
    if request.user.is_authenticated:

        # Detectar los datos de la incidencia.
        if request.POST.get('action') == 'accept_pending_vacations':
            vacations_id = request.POST.get('vacations_id')
            vacations = Calendar.objects.get(pk=vacations_id)

            vacations.state = 'Aceptadas'
            vacations.save()

            return HttpResponse(vacations.state)

    # En otro caso redireccionamos al login
    return redirect('/login')

# OBTENER DATOS PARA EDITAR LAS VACACIONES
def getEditingVacations(request):
    if request.user.is_authenticated:
        vacations_id = request.POST.get('vacations_id')
        user_id = request.POST.get('user_id')
        departament_id = request.POST.get('departament_id')

        vacations = Calendar.objects.raw("SELECT * FROM dashboard_calendar WHERE id="+vacations_id)
        tmpJson = serializers.serialize("json",vacations)
        tmpObj = json.loads(tmpJson)
        json_vacations = json.dumps(tmpObj)

        account = Account.objects.raw("SELECT * FROM dashboard_account WHERE id="+str(user_id))
        tmpJson = serializers.serialize("json",account)
        tmpObj = json.loads(tmpJson)
        json_account = json.dumps(tmpObj)

        departament = Departament.objects.raw("SELECT * FROM dashboard_departament WHERE id="+str(departament_id))
        tmpJson = serializers.serialize("json",departament)
        tmpObj = json.loads(tmpJson)
        json_departament = json.dumps(tmpObj)
...
# GUARDAR LAS FECHAS DE LAS VACACIONES UNA VEZ MODIFICADAS
def editVacations(request):
    #Comprobamos que el usuario está logueado
    if request.user.is_authenticated:

        # Detectar los datos de la incidencia.
        if request.POST.get('action') == 'edit_vacations':
            vacations_id = request.POST.get('vacations_id')
            start_date = request.POST.get('start_date')
            finish_date = request.POST.get('finish_date')
            user_id = request.POST.get('user_id')
            ...
            vacations.start_date = start_date
            vacations.finish_date = finish_date
            vacations.save()

    # En otro caso redireccionamos al login
    return redirect('/login')

# BORRAR PETICIONES DE VACACIONES QUE AUN NO HAN SIDO ACEPTADAS
def deletePendingVacations(request):
    #Comprobamos que el usuario está logueado
    if request.user.is_authenticated:

        # Detectar los datos de la incidencia.
        if request.POST.get('action') == 'delete_pending_vacations':
            vacations_id = request.POST.get('vacations_id')
            user_id = request.POST.get('user_id')
            ...
            user = Account.objects.get(pk=user_id)
            user.vacation_days = int(user.vacation_days) - int(work_days)
...
# BORRAR VACACIONES QUE HAN SIDO ACEPTADAS
def deleteAcceptedVacations(request):
    #Comprobamos que el usuario está logueado
    if request.user.is_authenticated:

        # Detectar los datos de la incidencia.
        if request.POST.get('action') == 'delete_accepted_vacations':
            vacations_id = request.POST.get('vacations_id')
            user_id = request.POST.get('user_id')
            work_days = request.POST.get('work_days')
            vacations = Calendar.objects.get(pk=vacations_id)

            vacations.removed=True
            vacations.save()
...
def saveTask (request):
    ...
    if request.user.is_authenticated:
        ...
        # Detectar la actualización de usuario.
        if request.POST.get('action') == 'save_task':
            user_id = request.POST.get('user_id')
            company_id = request.POST.get('company_id')
            departament_id = request.POST.get('departament_id')
            title = request.POST.get('title')
...
#GUARDAR DATOS DE RECORDATORIOS
def saveReminder (request):
    #Comprobamos que el usuario está logueado
    if request.user.is_authenticated:

        # Detectar la actualización de usuario.
        if request.POST.get('action') == 'save_reminder':
            user_id = request.POST.get('user_id')
            company_id = request.POST.get('company_id')
            departament_id = request.POST.get('departament_id')
            title = request.POST.get('title')
