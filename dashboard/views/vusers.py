...
from dashboard.models import Absence, Companies, Account, Profile, Function, Permission, Departament, Role, Notification, Calendar, CA, Invitation, Category, Language, CategoryLang, CheckInData
...
from dashboard.views import vubunet as ubunet
        ...
        if request.POST.get('action') == 'post':
            ...
            if(account.verified):
                ...
                if user is not None:
                    ...
                    #return render(request, 'dashboard/appDashboard.html')
...
def usersManagement(request):
    ...
    if request.user.is_authenticated:
        ...
        if request.user.owner == request.user.id:
            users = Account.objects.raw("SELECT dashboard_account.*, dashboard_ca.* FROM dashboard_account LEFT JOIN dashboard_ca ON dashboard_account.id=dashboard_ca.user_id AND dashboard_ca.company_id = " + str(company) + " where dashboard_account.owner = " + str(request.user.id) + " ORDER BY dashboard_ca.id DESC")
        else:
        ...
        profiles = Profile.objects.raw("SELECT dashboard_profile.* , dashboard_departament.name as dept_name FROM dashboard_profile INNER JOIN dashboard_departament ON dashboard_profile.departament_id = dashboard_departament.id WHERE dashboard_profile.company_id = " + str(company) + " ORDER BY dashboard_profile.departament_id DESC")
        departaments = Departament.objects.raw("SELECT * FROM dashboard_departament where dashboard_departament.company_id = " + str(company) + " ORDER BY id DESC")
        ...
        if request.POST.get('action') == 'add_user':
            ...
            #Descomentar Email
            mail = create_mail('angeliyovalderrubio3@gmail.com', subject, template_name, {'host': host,'verification_url': domain+'/user-invitation/'+m, 'company_name': company_obj.name})
...
def scheduledCalls(request):
    if request.user.is_authenticated:
        if request.POST.get('action')=='noti_notified':
        ...
        cursor = connection.cursor()    
        cursor.execute("SELECT DATE_PART('day',  NOW() - dashboard_notification.created_at)*24*60 + DATE_PART('hour', NOW() - dashboard_notification.created_at )*60 + DATE_PART('minute', NOW() - dashboard_notification.created_at) as minutes_passed, dashboard_notification.id, dashboard_notification.action, dashboard_notification.notified, dashboard_notification.contact_id, dashboard_notification.title, dashboard_notification.content, dashboard_notification.event_id FROM dashboard_notification WHERE NOT dashboard_notification.read AND dashboard_notification.user_id = "+str(user)+" AND dashboard_notification.company_id="+str(company))
...
def userEdition (request, user_id):
    ...
    if request.user.is_authenticated:
        ...
        calendars = Calendar.objects.raw("SELECT * FROM dashboard_calendar WHERE user_id='"+ str(user_ca.user_id) +"' and departament_id="+ str(user_ca.departament_id)+"")
        ...
        if request.POST.get('action') == 'update_user':
            ...
            for calendar in calendars:
                calendar.departament_id = departament_id
                calendar.save()
       ...
       # Consulta de los datos de la sesión
        ...
        if request.POST.get('action') == 'get_slots':
            ...
            if user != request.user.id:
                check = CheckInData.objects.raw("SELECT * FROM dashboard_checkindata WHERE dashboard_checkindata.user="+str(user)+" AND (dashboard_checkindata.check_in < TO_DATE('"+str(next_year)+"-"+str(next_month)+"-01', 'YYYY-MM-DD') AND dashboard_checkindata.check_in >= TO_DATE('"+str(year)+"-"+str(month)+"-01', 'YYYY-MM-DD')) ORDER BY dashboard_checkindata.check_in ASC")
                tmpJson = serializers.serialize("json",check)
                tmpObj = json.loads(tmpJson)
                json_check = json.dumps(tmpObj)

                absence = Absence.objects.raw("SELECT * FROM dashboard_absence WHERE dashboard_absence.user="+str(user)+" AND dashboard_absence.deleted = 0 AND dashboard_absence.state = 1 AND (dashboard_absence.start < TO_DATE('"+str(next_year)+"-"+str(next_month)+"-01', 'YYYY-MM-DD') AND dashboard_absence.finish >= TO_DATE('"+str(year)+"-"+str(month)+"-01', 'YYYY-MM-DD'))")
                tmpJson = serializers.serialize("json",absence)
                tmpObj = json.loads(tmpJson)
                json_absence = json.dumps(tmpObj)
            else:
                check = CheckInData.objects.raw("SELECT * FROM dashboard_checkindata WHERE dashboard_checkindata.user="+str(request.user.id)+" AND (dashboard_checkindata.check_in < TO_DATE('"+str(next_year)+"-"+str(next_month)+"-01', 'YYYY-MM-DD') AND dashboard_checkindata.check_in >= TO_DATE('"+str(year)+"-"+str(month)+"-01', 'YYYY-MM-DD')) ORDER BY dashboard_checkindata.check_in ASC")
                tmpJson = serializers.serialize("json",check)
                tmpObj = json.loads(tmpJson)
                json_check = json.dumps(tmpObj)

                absence = Absence.objects.raw("SELECT * FROM dashboard_absence WHERE dashboard_absence.user="+str(request.user.id)+" AND dashboard_absence.deleted = 0 AND dashboard_absence.state = 1 AND (dashboard_absence.start < TO_DATE('"+str(next_year)+"-"+str(next_month)+"-01', 'YYYY-MM-DD') AND dashboard_absence.finish >= TO_DATE('"+str(year)+"-"+str(month)+"-01', 'YYYY-MM-DD'))")
                tmpJson = serializers.serialize("json",absence)
                tmpObj = json.loads(tmpJson)
                json_absence = json.dumps(tmpObj)
    ...
    if request.user.is_authenticated:
       # Consulta de los datos de la sesión
        ...
        if departament == departament2 or request.user.id == 18 or request.user.id == 64:
            return render(request, 'checkIn/checkIn.html',{'role': ca.role,'user_id':user_id, 'user_name':user_name, 'year':year, 'month':month})
        else:
            return render(request, 'accessDenied/accessDenied.html')
       ...
       # Consulta de los datos de la sesión
        ...
        if request.POST.get('action') == 'change_month':
            ...
            if request.user.id == 18 or request.user.id == 64 or request.user.id == 25 or request.user.id == 36 or request.user.id == 43:
                ...
                users = Account.objects.raw("SELECT dashboard_account.*, dashboard_ca.* FROM dashboard_account JOIN dashboard_ca ON dashboard_account.id = dashboard_ca.user_id WHERE dashboard_account.is_active = true AND dashboard_ca.departament_id = "+str(departament))
                tmpJson = serializers.serialize("json",users)
                users = json.loads(tmpJson)
