...
def get_agents(request):
    resultado = Account.objects.raw("SELECT dashboard_account.* FROM dashboard_account JOIN dashboard_ca ON dashboard_account.id=dashboard_ca.user_id WHERE role=2")
...
def checkData(request):
    if request.user.is_authenticated:
        ...
        if request.POST.get('action') == 'check_in_data':
            check = CheckInData.objects.raw("SELECT * FROM dashboard_checkindata WHERE dashboard_checkindata.user="+str(request.user.id)+" ORDER BY check_in DESC LIMIT 1")
        ...
        if request.POST.get('action') == 'tooltip_data':
            ...
            ci_list = []
            check = CheckInData.objects.raw("SELECT * FROM dashboard_checkindata WHERE dashboard_checkindata.user="+str(request.user.id)+" ORDER BY id DESC LIMIT 10")
