...
def loadCompanies(context):
    companies = Companies.objects.raw('SELECT dashboard_companies.*, dashboard_ca.on_use FROM dashboard_companies JOIN dashboard_ca ON dashboard_ca.company_id = dashboard_companies.id WHERE dashboard_ca.user_id = '+str(context['request'].user.id))
