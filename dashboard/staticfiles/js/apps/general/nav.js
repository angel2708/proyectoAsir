$.ajax({
    ...
    success: function (json) {
        ...
        let notf_count = notifications.length;

        if(notf_count==0){
            $('.notification-count').hide();
        }

        $('.notification-count').html(notf_count);
        $('.new-notif-count').html(notf_count+' NUEVAS');

        for (i in notifications){
            let time_passed = "Hace ";

            //console.log(notifications[i]['minutes_passed']);
            if(notifications[i]['minutes_passed'] < 0)
                time_passed += '0 minutos.';
            else if(notifications[i]['minutes_passed'] < 60)
                time_passed += notifications[i]['minutes_passed']+' minutos.';
            else if(notifications[i]['minutes_passed'] < 1440)
                time_passed += Math.trunc(notifications[i]['minutes_passed']/60)+' horas.';
            else
                time_passed += Math.trunc(notifications[i]['minutes_passed']/1440)+' días.';

            $('.notification-container').append(
                '<li notif-id="'+notifications[i]['id']+'" class="scrollable-container media-list notification-elem">'+
                '    <div>'+
                '        <a class="hide-notification" style="right:5px; top:10px;position:absolute;" notif-id="'+notifications[i]['id']+'"><i class="fa fa-close"></i></a>'+
                '        <a target="_blank" class="notification-link" action="'+notifications[i]['action']+'" notification-id="'+notifications[i]['id']+'" client-id="'+notifications[i]['contact_id']+'"><div class="media"><div class="media-left align-self-center"><i class="'+notifications[i]['icon']+' icon-bg-circle bg-'+notifications[i]['color']+'"></i></div>'+
                '        <div class="media-body">'+
                '            <h6 class="media-heading">'+notifications[i]['title']+'</h6>'+
                '            <p class="notification-text font-small-3 text-muted">'+notifications[i]['content']+'</p><small>'+
                '                <time class="media-meta text-muted">'+time_passed+'</time></small>'+
                '        </div></div></a>'+
                '    </div>'+
                '</li>'
            );
...
setInterval(function(){
    ...
    $.ajax({
        ...
        success:function(json){
            ...
            for (i in notifications){
                let time_passed = "Hace 0 minutos";

                if(notifications[i]['minutes_passed']+120>0){
                    time_passed = "Hace "+notifications[i]['minutes_passed']+" minutos";
                }

                if(notifications[i]['minutes_passed']+120>=-5){
                    let f = false;

                    $('.notification-elem').each(function(){
                        if(parseInt($(this).attr('notif-id')) == notifications[i]['id']){
                            f=true;
                        }
                    });
                    ...
                    if(!f){
                        ...
                        cv++;
                        if(notifications[i]['notified']==false){
                            if(!first_loop){
                                if (role == '6') {
                                    toastr.info(notifications[i]['content'],notifications[i]['title'])
                                } else {
                                    toastr.info(notifications[i]['content'],notifications[i]['title'], {timeOut: 0})
                                }
                            }else{
                                new_notif++;
                            }
            ...
            let notf_count = cv+parseInt($('.notification-count').html());

            if(notf_count!=0){
                $('.notification-count').slideDown();
            }

            $('.notification-count').html(notf_count);
            $('.new-notif-count').html(notf_count+' NUEVAS');

            if(first_loop && new_notif>0){
                if (role == '6') {
                    toastr.info('Tienes '+new_notif+' notificaciones nuevas.');
                } else {
                    toastr.info('Tienes '+new_notif+' notificaciones nuevas.', {timeOut: 0});
                }
            }
...
$(document).on('click', '.hide-notification', function () {
    let li_ele = $(this).closest('li');
    li_ele.closest('ul').parent().addClass('show');
    let notification = $(this).attr('notif-id');    
    ...
    $.ajax({
        ...
        success: function () {
            li_ele.closest('ul').parent().addClass('show');
            li_ele.slideUp("normal", function() { $(this).remove(); } );
            let n_count = parseInt($('.notification-count').html());
            n_count--;

            $('.notification-count').html(n_count);
            $('.new-notif-count').html(n_count+' NUEVAS');
        },
        error: function (xhr, errmsg, err) {
            console.log(xhr.status + ": " + xhr.responseText); // provide a bit more info about the error to the console
        }
...
$(document).on('click', '.hide-all-notification', function () {
    swal.fire({
        title: "¿Desea borrar todas las notificaciones?",
        type: 'question',
        showCancelButton: true,
        cancelButtonText: 'NO',
        ...
        if (resp['value'] == true) {
            $(".notification-elem").each(function(){
                ...
                $('.new-notif-count').html('0 NUEVAS');
                $(this).remove()
            })
            toastr.success('Operación realizada con éxito', 'Se han eliminado todas las notificaciones.', { positionClass: 'toast-top-right', containerId: 'toast-bottom-right', "showMethod": "slideDown", "hideMethod": "slideUp", timeOut: 3000 });
        }
    })
})
